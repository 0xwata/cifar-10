...
def unpickle(batch_id):
    with open('data_batch_'+str(batch_id), 'rb') as f:
        dataset = pickle.load(f, encoding='bytes')
        return dataset

def load_cfar10_batch(batch_id):
    batch = unpickle(batch_id)

    features = batch[b'data'].reshape((len(batch[b'data']), 3, 32, 32)).transpose(0, 2, 3, 1) #各画像のピクセル値を取得
    labels = batch[b'labels'] #各画像のラベルを取得
...
def main():
    ...
    for batch_i in range(1, n_batches+1):
        ...
        index_of_validation = int(len(features)*0.1) #validationに使うデータセットの数を計算

        #preprocess 90% of the whole batch
    ...
    # Initializing the variables
        ...
        for epoch in range(epochs):
            ...
            for batch_i in range(1, n_batches + 1):
                ...
                batch_features_and_labels = batch_features_labels(batch_features, batch_labels, batch_size)

                for batch_features, batch_labels in batch_features_and_labels:
                    sess.run(optimizer,
                        feed_dict={
                            x: batch_features,
                            y: batch_labels,
